...
<template>
  <div class="common-grid-drag-cmp">
    ...
    <el-row :style="styleWidth" class="u-f grid-row">
      <el-col :span="obj.span" class="content-layout-item">
        <!-- 列{{key+1}} -->
        <vuedraggable
        ...
        >
          ...
          >
            <div
              ...
              :key="index + 1"
              :class="[`cmpBox_${item.minUnicode}`, 'cmpItemBox']"
            >
              <!-- <el-button type="primary" size="mini">{{item.controlName}}</el-button> -->
              <div
                :class="[
                  'cmp-item',
                  `cmp-item-${item.minUnicode}`,
                  'atris-selectable',
                  'atris-hoverable',
                ]"
                :data-minunicode="`${item.minUnicode}`"
              ...
              >
                ...
                <div
                  :class="[
                    'cmp-item-handlerBox',
                    `cmp-item-handler-${item.minUnicode}`,
                  ]"
                  :data-minunicode="`${item.minUnicode}`"
                >
                  <el-tooltip
    ...
    handlerClickCopy(e, obj, index) {
      ...
      if (res) {
        if (res.parentCode === "allCode") {
          this.pageSetTotalData.pageSetTotalDataList.splice(
            ++res.levelIndex,
            0,
            resultObj
          );
