...
<template>
  <div  class="excuteEventList-cmp" v-loading='loading'> 
    <!--事件配置table列表--start-->
    ...
    <!---事件执行弹框-start---->
    <div class="exceteEventBox" v-if="showExcuteEventDialog">
      <el-dialog
        title="执行事件"
        width="100%"
        fullscreen
        append-to-body
        :close-on-click-modal="false"
        :visible.sync="showExcuteEventDialog"
      >
        <event-excute-first-cmp 
          ref="eventExcuteCmp"
          :eventObj="currentExcuteEvent"
          :showDialog.sync="showExcuteEventDialog"
        >
        </event-excute-first-cmp>
      </el-dialog>        
    </div>
  ...
  export default {
    ...
    data(){
      return {
        ...
        showExcuteEventDialog: false, // 控制执行事件弹框的显示/隐藏
        currentExcuteEvent: {}, // 当前执行的事件对象
        tableData: [],
        form: {
          BeginDate: '',
          EventCode: '',
    ...
    methods: {
      ...
      handlerExcute(obj){
        debugger
        this.currentExcuteEvent = obj
        this.showExcuteEventDialog = true
      },
      // 新增
      addEvent(){
