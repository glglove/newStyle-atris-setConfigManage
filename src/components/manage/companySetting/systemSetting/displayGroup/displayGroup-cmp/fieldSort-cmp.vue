    ...
    <div 
      ...
      v-loading="loading">
        ...
        <div class="TipBox" v-if="fieldList.length>0">
          <span class="sortTipTit" style="color:red;marin-left:4px">拖拽排序</span>
        </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created () {
        this._CompFieldList(this.obj.TeamCode , '', this.propState, 655533, 1)
    },
    mounted () {
    ...
    methods: {
        _CompFieldList(teamCode, tableCode, state, pageSize, pageNum){
            this.loading = true
            CompFieldList(teamCode, '', state, pageSize, pageNum).then(res => {
                this.loading = false
