                ...
                <div class="treeBox">
                    ...
                    >
                            <span :class="['custom-tree-wrap'] " slot-scope="{ node, data }">
                                ...
                                >     
                                    <span class="nameBox">
                                        <el-form-item v-if="data.isEditing" prop="usergroupname" :rules="data.itemRules">
                                            <el-input
                                                :class="['name-input', data.usergroupcode === currentUserGroupCode?'empty':'']"
                                                v-model="data.usergroupname"
                                                placeholder="请输入"
                                                @change="nameInputChange(node, data)"
                                            >
                                            </el-input>
                                        </el-form-item>                                
                                        <span v-else class="name-text ellipsis1">{{ data.usergroupname }}</span>                            
                                    </span>
...
export default {
    ...
    methods:{
        ...
        // 保存
        _saveUserGroup(node, data){
            debugger
            this.loading = true
            this._signChanged([data])
            saveUserGroup(this.treeData).then(res => {
                this.loading = false
        ...
        cancel(node, data){
            debugger
            this.currentUserGroupCode = data.usergroupcode
            if(!data.usergroupname){
        ...
        remove(node, data) {
            ...
            const index = childrenList.findIndex(d => d.usergroupcode === data.usergroupcode);
            debugger
            if(data.id){
            ...
            if(arr && arr.length){
                arr.forEach((item, key) => {
                    let res = new Promise((resolve, reject) => {
                        item.sortId = ++key
                        console.log(that.$refs[`dataForm_${item.usergroupcode}`])
                        debugger
                        that.$refs[`dataForm_${item.usergroupcode}`].validate(valid => {
                            debugger
                            if(valid){ 
                                resolve(valid)   
                            }else {
                                reject(new Error(`${item.usergroupcode}验证失败`))
                                return false
                            }
                        })         
                    })
